...
/**
 * ONE product card, used by the homepage rails, all three shop grids and the
 * related-products strip. Because every grid renders this same function, all 38
 * products present identically wherever they appear (Spec 2, 6.2).
 */
function productCard(p, opts) {
  ...
  return `<article class="pcard">
            ...
            <div class="pcard__quick">
              <button class="btn btn--primary btn--sm btn--block" type="button" data-quick-add="${p.id}">
                ${icon('cart', 'icon icon--sm')} Add to Cart
              </button>
            </div>
            <div class="pcard__body">
              ...
              <div class="pcard__foot">
                ...
                <span class="meta">${esc(p.isGarment ? p.colour : p.netQuantity || '')}</span>
              </div>
            </div>
          </article>`;
}
